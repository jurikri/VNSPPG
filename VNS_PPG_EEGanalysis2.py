        ...
        for ch in range(2):
            ...
            for bi in range(len(msbins)):
                ...
                if tmp.shape[0] == window_size:
                    ...
                    # db = 10 * np.log10(np.power(nmr - nmr_pre, 2)) # nmr - dB
                    db = 10 * np.log10(np.power(tmp2 - tmp_pre, 2)) # non nmr - dB
                    # db_nmr = db / np.mean(np.abs(db))
                    mssave_array[label_n][ch].append(db)
                    # mssave_array[label_n][ch].append(nmr) # non db
